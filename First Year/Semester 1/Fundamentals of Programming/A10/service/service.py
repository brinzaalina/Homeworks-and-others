...
class GradeService:
    ...
    def compare_elements(elem1, elem2):
        if float(elem1.grade) >= float(elem2.grade):
            return True
        else:
            return False
    ...
    def filter_by_grade(self):
        ...
        passing_grades = IterableStructure.filter(grades, lambda element: element.grade >= 5)
        passing_grades_sorted = IterableStructure.gnome_sort(passing_grades, self.compare_elements)
        return passing_grades_sorted
