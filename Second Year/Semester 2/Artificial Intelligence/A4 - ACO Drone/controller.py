        ...
        for i in range(self.drone_map.number_of_sensors):
            for j in range(self.drone_map.number_of_sensors):
                for spent_energy in range(MAX_SENSOR_COVERAGE + 1):
                    ...
                    self.pheromone_matrix[i][j][spent_energy] = (1 - self.rho) * self.pheromone_matrix[i][j][spent_energy] + self.rho * self.initial_pheromone_matrix[i][j][spent_energy]
        for ant in population:
