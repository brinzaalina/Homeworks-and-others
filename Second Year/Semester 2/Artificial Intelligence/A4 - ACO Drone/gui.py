...
def moving_drone(current_map, path, battery, speed=0.5):
    ...
    drone = pygame.transform.scale(pygame.image.load("drona.png"), (COLUMN_SIZE, ROW_SIZE))
    ...
    sensor = pygame.transform.scale(pygame.image.load("virtual-glasses.png"), (COLUMN_SIZE, ROW_SIZE))
    battery += 1
    running = True
    ...
    for cell_i in range(len(path)):
        ...
        print("Battery left: {}".format(battery))
        ...
        screen.blit(drone, (path[cell_i][1] * COLUMN_SIZE, path[cell_i][0] * ROW_SIZE))
        pygame.display.flip()
        time.sleep(speed)
    close_pygame()
...
def image(current_map, colour=BLUE, background=WHITE):
    img = pygame.Surface((current_map.m * COLUMN_SIZE, current_map.n * ROW_SIZE))
    brick = pygame.Surface((COLUMN_SIZE, ROW_SIZE))
    brick.fill(colour)
    img.fill(background)
    for i in range(current_map.n):
        for j in range(current_map.m):
            if current_map.surface[i][j] == 2:
                img.blit(brick, (j * COLUMN_SIZE, i * ROW_SIZE))
    return img
